...
def ping_auction_service():
    ...
    return handle_auction_service_request('GET', '/ping', variant=1, load_balancer=load_balancer)

@ping_blueprint.route('/bidder', methods=['GET'])
def ping_bidder_service():
    load_balancer = request.args.get('load_balancer', 'round_robin')    
    return handle_bidder_service_request('GET', '/ping', variant=2, load_balancer=load_balancer)